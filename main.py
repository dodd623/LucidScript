      ...
      <body>
        <div class="wrap">
          ...
          <div class="card">
            ...
            <small>Prefer the API? Try <a href="/docs">/docs</a>. Also see <a href="/ui_async">/ui_async</a> and <a href="/ui_youtube">/ui_youtube</a>.</small>
          </div>
        </div>
      </body>
    </html>
...
def _fetch_youtube_captions_text(url: str, preferred_lang: Optional[str]) -> str:
    if yt_dlp is None:
        return ""
    tmp_dir = tempfile.mkdtemp(prefix="ls_ytsub_")
    outtmpl = os.path.join(tmp_dir, "%(id)s.%(ext)s")
    langs = []
    if preferred_lang:
        langs.append(preferred_lang)
        langs.append(preferred_lang.split("-")[0])
    langs += ["en", "en-US", "en-GB", "en-US,en-GB", "en.*", ".*"]
    ydl_opts = {
        "skip_download": True,
        "writesubtitles": True,
    ...
    try:
        with yt_dlp.YoutubeDL(ydl_opts) as ydl:
            info = ydl.extract_info(url, download=True)
            vid = info.get("id")
            candidates = glob.glob(os.path.join(tmp_dir, f"{vid}*.vtt")) or glob.glob(os.path.join(tmp_dir, "*.vtt"))
            if candidates:
                path = candidates[0]
    except Exception:
        path = ""
    text = _vtt_to_text(path) if path else ""
    try:
        if path and os.path.exists(path):
            os.remove(path)
        if os.path.exists(tmp_dir):
            for p in pathlib.Path(tmp_dir).glob("*"):
                try:
...
async def export_docx_from_youtube(
    ...
    audio_path = None
    try:
        cap_text = _fetch_youtube_captions_text(url, language)
        if not cap_text:
